...
*/	
	...
	$conexion = oci_connect("$usuario_conn","$pass_conn", "$ip/$instancia");
		...
		$sql = "SELECT CODIGO, DESCRIP, DOMICILIO FROM COS_ITEM WHERE CLIENTE='$codigo_cliente'";
		...
		if( $ok == true )
		{
			...
			if( $obj = oci_fetch_object($stmt) ){
				...
				do
					{	
						$codigo_item[$i] =  $obj->CODIGO;
						$descripcion_item[$i] =  $obj->DESCRIP;
						$domicilio_item[$i] =  $obj->DOMICILIO;
						$i++;
